...
AUTOMACOES: dict[str, str] = {}
AUTOMACAO_INICIADA = 'Iniciada'
AUTOMACAO_FINALIZADA = 'Finalizada'


def criar_automacao(nome):
    id_automacao = str(uuid.uuid4())
    AUTOMACOES[id_automacao] = {
        'nome': nome,
...
def verificar(request, id_automacao):
    return http.JsonResponse({'status': AUTOMACOES[id_automacao]['status']})


def automacao_teste(request):
    id_automacao = criar_automacao('Automação teste')
...
def tarefa(id_automacao):
    ...
    AUTOMACOES[id_automacao]['status'] = AUTOMACAO_FINALIZADA
